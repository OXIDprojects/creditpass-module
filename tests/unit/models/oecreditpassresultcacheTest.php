...
class Unit_Models_oeCreditPassResultCacheTest extends OxidTestCase
{
    ...
    public function testGetData()
    {
        ...
        $oDB = $this->getMock(
            'oeCreditPassResponseCacheDbGateway', array('load', 'setAddressId', 'setPaymentId'), array(), '', false
        );
        $oDB->expects($this->once())->method('load')->will($this->returnValue($sXML));

        $oCreditPassResultCache = $this->getMock(
            'oeCreditPassResultCache', array('_getDbGateway', '_deleteExpCache')
        );
    ...
    public function testGetDataRemoveOldData()
    {
        ...
        $oDB = $this->getMock(
            'oeCreditPassResponseCacheDbGateway', array('load', 'delete', 'setAddressId', 'setPaymentId'), array(), '',
            false
        );
        $oDB->expects($this->once())->method('load')->will($this->returnValue(false));
        $oDB->expects($this->once())->method('delete')->with('1970-01-01 01:00:05'); //86405 - 1*86400

        $oCreditPassResultCache = $this->getMock('oeCreditPassResultCache', array('_getDbGateway', '_getTime'));
    ...
    public function testGetCheckCacheTimeout()
    {
        ...
        $oCreditPassResultCache = new oeCreditPassResultCache();
    ...
    public function testStoreData()
    {
        ...
        $oDB = $this->getMock('oeCreditPassResponseCacheDbGateway', array('save'));
    ...
    public function testGetRejectedPaymentIds()
    {
        ...
        $oDB = $this->getMock('oeCreditPassResponseCacheDbGateway', array('loadPaymentIdsByAnswer', 'setAddressId'));
        ...
        $oCreditPassResultCache = $this->getMock('oeCreditPassResultCache', array('_getDbGateway'));
